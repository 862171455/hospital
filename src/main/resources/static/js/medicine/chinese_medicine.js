layui.use(['table', 'layer', 'jquery'], function(){
    ...
    table.render({
        ...
        ,cols: [[ //表头
            ...
            ,{field:'chi_no', title:'序号',width:"10%", sort: true, fixed: 'left',align:"center",style:'color:#000;',fixed: true,templet:'<div><a class="layui-table-link">{{d.LAY_TABLE_INDEX+1}}</a></div>'}
            ,{field:'chi_name', title: '中药名称', width:"20%",align:"center"}
    ...
    table.on('tool(test)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if(layEvent === 'detail'){ //查看
            layer.open({
                ...
                anim: 6,
                title: "中药详情",
                content: '/for/chinese_detail',
                zIndex: layer.zIndex, //重点1
                success: function(layero){
                    layer.setTop(layero); //重点2
                    var body=layui.layer.getChildFrame("body");
                    body.find("[name='chi_name']").val(data.chi_name);
